...
dataM = read.csv("Motifs_DIR_118-Bus.csv")

#=== 4N ===#
dm01 = dataM[,c(2,3,4,6,7,8,11)]
...
D1_V1          =  abs(diff(dm01$X1, lag = 1)) # Death #
...
D1_V2          = abs(diff(dm01$X2, lag = 1)) # Death
Time1_V2       = dm01[-1,1]           
...
D1_V4          = abs(diff(dm01$X4, lag = 1)) # Death
Time1_V4       = dm01[-1,1]    
...
D1_V5          = abs(diff(dm01$X5, lag = 1)) # Death
Time1_V5       = dm01[-1,1]   
...
D1_V6          = abs(diff(dm01$X6, lag = 1)) # Death
Time1_V6       = dm01[-1,1]   
...
D1_V9          = abs(diff(dm01$X9, lag = 1)) # Death
Time1_V9       = dm01[-1,1]   
df9            = data.frame(D1_V9,Time1_V9)
dm9            = df9[rep(rownames(df9), df9$D1_V9), ]
colnames(dm9)  = c("Death", "Time") # Making column name same
dm9$event1     = 1

#========= Non-parametric Survival Model ==========#
NPsV9          =  survfit(Surv(Time, event1) ~ 1,  type="kaplan-meier", conf.type="log", data=dm9)

#========= Parametric Survival Model ==========#
PsV9           = survreg(Surv(Time, event1) ~ 1,  data=dm9, dist="exponential")
lamda9         = exp(-PsV9$coefficients[[1]]) #lemda=exp(-Intercept)
S_V9           = exp(-lamda9*t) 


#========= Plot nonparametric vs parametreic =========#
# plot(NPsV9,conf.int=FALSE,lwd=2,lty=1,xlab="Time",ylab="Survival Probability", main="V9",xlim = c(0,80), ylim = c(0,1))
# lines(S_V9,col="red",lwd=2,lty=1)
# legend('topright',c( "KME","MLE"), lty=c(1,1), lwd=c(2,2), col=c("black","red"))    
...
for (i in (1:length(t))) { #i=1
  ...
  PAc =  ((1-exp(-lamda1*t[i]))*(1-exp(-lamda2*t[i]))*(1-exp(-lamda4*t[i]))* (1-exp(-lamda5*t[i]))*(1-exp(-lamda6*t[i]))*(1-exp(-lamda9*t[i])))
  
  Mn = min((1-exp(-lamda1*t[i])),(1-exp(-lamda2*t[i])),(1-exp(-lamda4*t[i])), (1-exp(-lamda5*t[i])),(1-exp(-lamda6*t[i])),(1-exp(-lamda9*t[i])))
...
D_d1 = data.frame(t,S_V1,S_V2,S_V4,S_V5,S_V6,S_V9,DRt)
colnames(D_d1) = c("V_Remv", "S_V1", "S_V2", "S_V4", "S_V5", "S_V6", "S_V9", "DRt")
write.csv(D_d1, "Motifs_DIR_3N_80Reliab.csv")
...
res3 = read.csv("SWProp_DIR_118.csv")
...
plot(NPsV1,conf.int=FALSE, xlab="Number of node(s) removed",ylab="Load served/Robustness metric", xlim = c(0,80), ylim = c(0,1),
     main="Directed: Robustness metrics (3-N motifs,TDA only) & Load served",  lty = 1, lwd = 2, col = "purple")
lines(NPsV2,conf.int=FALSE,   type = "o", pch = 7, lty = 1, lwd = 2, col = "red")
lines(NPsV4,conf.int=FALSE,   type = "l",          lty = 4, lwd = 2, col = "cyan")
lines(NPsV5,conf.int=FALSE,   type = "l",          lty = 1, lwd = 3, col = "green")
lines(NPsV6,conf.int=FALSE,   type = "l",          lty = 2, lwd = 2, col = "blue")
lines(NPsV9,conf.int=FALSE,   type = "s",          lty = 3, lwd = 2, col = "black")
lines(c(0:80),LS_vals$LS,     type = "o", pch = 21,lty = 1, lwd = 1, col = "orange")
lines(res1$V_Remv, res1$Wass01, type = "l",        lty = 6, lwd = 3, col = "brown")
grid();
#par(xpd = T)
legend("topright", bty = "n", inset = 0.005,
       c(expression(paste(M[1])), expression(paste(M[2])) , expression(paste(M[4])),expression(paste(M[5])),expression(paste(M[6])),expression(paste(M[9])),expression(paste(Load," " ,served)),expression(paste(1," - ",Delta ,W[2](D[0],D[P])))),
...
plot(res1$V_Remv, res1$Wass01, xlab="Number of node(s) removed",ylab="Load served/Robustness metric", xlim = c(0,80), ylim = c(0,1),
     main = "Directed: Robustness metrics (3-N motifs) & Load served", type = "l",lty = 1, lwd = 2, col = "purple")
...
legend("bottomleft", bty = "n", inset = 0.005,
       c(expression(paste(1," - ",Delta ,W[2](D[0],D[P]))),expression(paste(Load," ",served)),expression(paste(MR)), expression(paste(1, "-", Delta, S)), expression(paste(Delta, LC)), expression(paste(Delta, APL))),
       cex = 0.9,lty=c(1,1,6,2,1,3),pch = c(NA,5,NA,NA,1,NA),lwd=c(2,1,3,1,2,3), col=c("purple","orange", "brown","red", "green", "blue")) 

par(mar = c(5, 4, 4, 2) + 0.1)


plot(t, CLUS_vals[,2], type = "o", col = "blue", main = "Normalized clustering coefficient", xlab = "Number of node(s) removed", ylab = "Normalized value")
plot(t, DIAM_vals[,2], type = "o", col = "red", main = "Normalized diameter", xlab = "Number of node(s) removed", ylab = "Normalized value")

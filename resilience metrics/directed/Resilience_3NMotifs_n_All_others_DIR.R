...
for(i in 1:fnm){
  ...
  gg_dir = graph_structure_under_sequence_row_attack
...
M_conc = m2/n2 # Motif concentration



resultsN = data.frame(c(0:(fnm - 1)), m2,  M_conc)
write.csv(resultsN, "Motifs_DIR_118-Bus.csv")
...
write.csv(resultsN, "TDA_DIR_118.csv")
...
write.csv(resultsN, "GCC_DIR_118.csv")
...
write.csv(resultsN, "LC_DIR_118.csv")
...
write.csv(resultsN, "SWProp_DIR_118.csv")
...
Flow_matrix = new_case118_result1[which(colnames(new_case118_result1)=="F_1"):which(colnames(new_case118_result1)=="F_186")] 
FlowShs_mat = apply(Flow_matrix, 1, function(x){sum(abs(x))})
FL_value    = FlowShs_mat/FlowShs_mat[1] 

write.csv(FL_value, "FL_value.csv")
